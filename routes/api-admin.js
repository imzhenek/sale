...
router.post('/models', upload.fields([{ name: 'photo_main', maxCount: 1 }, { name: 'photos', maxCount: 8 }]), (req, res) => {
  const { name, category, height, bust, weight, age, city, bio, status } = req.body;
  ...
  const info = db.prepare(`
    INSERT INTO models (slug, name, category, height, bust, weight, age, city, bio, photo_main, photos_json, status)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(slug, name, category || 'women', height || null, bust || null, weight || null, age || null, city || null, bio || null, photoMain, JSON.stringify(photos), status || 'active');
...
router.put('/models/:id', upload.fields([{ name: 'photo_main', maxCount: 1 }, { name: 'photos', maxCount: 8 }]), (req, res) => {
  ...
  db.prepare(`
    UPDATE models SET name=?, category=?, height=?, bust=?, weight=?, age=?, city=?, bio=?, photo_main=?, photos_json=?, status=?
    WHERE id = ?
  `).run(name, category || 'women', height || null, bust || null, weight || null, age || null, city || null, bio || null, photoMain, JSON.stringify(photos), status || 'active', req.params.id);
