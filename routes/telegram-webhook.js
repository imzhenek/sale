...
async function handleCallback(cq) {
  ...
  try {
    await sendMessage(booking.client_telegram_id,
      newStatus === 'confirmed'
        ? `✅ Ваша заявка на съёмку${model ? ' с ' + model.name : ''} подтверждена. Мы свяжемся с вами для уточнения деталей.`
        : `❌ К сожалению, по вашей заявке${model ? ' на съёмку с ' + model.name : ''} — отказ. Можете отправить новую заявку с другими датами.`
    );
  } catch (e) {
